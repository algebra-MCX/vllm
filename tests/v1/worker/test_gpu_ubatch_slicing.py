...
@pytest.mark.parametrize("n", [2, 3, 32, 56, 60, 63])
def test_conditional_decode_preserves_token_position_and_kv_mapping(n):
    """Inserting padding must preserve every real row and isolate KV writes."""
    buffers = InputBuffers(128, 128, torch.device("cpu"))
    batch = _make_input_batch([1] * n, [100 + i for i in range(n)], buffers, 128, 128)
    batch.input_ids.copy_(torch.arange(128))
    batch.positions.copy_(torch.arange(128) + 100)
    blocks = torch.arange(128 * 4).reshape(128, 4)
    slots = torch.arange(128).reshape(1, 128) + 1000
    ...
    indices = torch.cat((torch.arange(first), torch.arange(64, 64 + second)))
    ...
    assert batch.is_padding.sum() == 128 - n
    ...
    compact_conditional_output(output, (first, second))
    torch.testing.assert_close(output[:n, 0], torch.arange(n).int())


@pytest.mark.parametrize("n", [0, 1, 64, 80])
...
@pytest.mark.skipif(not torch.cuda.is_available(), reason="needs a graph pool")
@pytest.mark.parametrize("loads", [[63, 97], [63, 127], [63, 64], [32, 96]])
