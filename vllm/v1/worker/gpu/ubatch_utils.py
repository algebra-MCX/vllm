...
class UBatchRunner:
    ...
    def __init__(
        ...
        # Experimental scope: the measured Qwen3 MoE decode configuration only.
        self.conditional_real_split = (
            ...
            and all(
                group.backend.get_name() == "FLASH_ATTN"
    ...
    def prepare(
        ...
        if (
            ...
            and not for_capture
            and input_batch.num_tokens_after_padding == 128
            and 2 <= input_batch.num_tokens < 64
        ):
            real_split = stage_conditional_decode(
                input_batch, block_tables, slot_mappings
            )
            ubatch_slices = [
                UBatchSlice(slice(i * 64, (i + 1) * 64), slice(i * 64, (i + 1) * 64))
                for i in range(2)
            ]
        else:
        ...
        for i, ubatch_slice in enumerate(ubatch_slices):
            ...
            if real_split is not None:
                count = real_split[i]
                self.ubatch_query_start_loc[i][count + 1 : 65].fill_(count)
                self.ubatch_seq_lens[i][count:64].zero_()
                query_cpu = np.minimum(np.arange(65, dtype=np.int32), count)
                seq_cpu = torch.zeros(64, dtype=torch.int32)
                seq_cpu[:count] = ubatch.seq_lens_cpu_upper_bound
                ubatch = replace(
                    ubatch,
                    num_reqs_after_padding=64,
                    num_tokens_after_padding=64,
                    query_start_loc=self.ubatch_query_start_loc[i][:65],
                    query_start_loc_np=query_cpu,
                    seq_lens=self.ubatch_seq_lens[i][:64],
    ...
    def abort_pending_run(self) -> None:
        ...
        try:
            ...
            logger.exception("Failed to join microbatch threads after an abort")
        finally:
            self._pending_finish = None
...
def maybe_build_ubatch_runner(
    ...
    logger.info_once("Dual batch overlap is enabled.")
    return UBatchRunner(
